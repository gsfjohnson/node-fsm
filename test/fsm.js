...
describe('FiniteStateMachine', () =>
{
  ...
  describe('object parameters: { next, start, id }', () =>
  {
    const next = new Map();
    next.set('WAITING', new Set(['ACTION']));
    next.set('ACTION', new Set(['WAITING']));
    const start = 'WAITING';
    const id = 'id';

    it('new FiniteStateMachine(params) should succeed', () =>
    {
      const params = { next, start, id };
      let res = new FSM.FiniteStateMachine(params);

      fsm = res;
    });

    it('fsm.next(ACTION) should succeed', function()
    {
      if (!fsm) this.skip();
